 ...
 * Prompt 6:
 *
 * Create a loop that counts from 0 to 50 (inclusive) in multipgit les of 2 (instead of 1),
...
function prompt8() { //Math.random generates float num between 0-1 non-inclusive, so mult by max to set max val
  ...
  } else if(30 < someNumber && someNumber < 60){
...
function prompt11() {
  // YOUR CODE HERE
  nums.sort(compare);// sorts Alphabetically so modifications needed unless we use more advanced sorting Algos
  function compare(a, b){
    return a-b;
  }
  console.log(nums); //  confirm sort
  let median = Math.floor(nums.length / 2); // find median after sort
  console.log(median);
}
// COMMENT IN THE BELOW LINE OF CODE TO TEST YOUR OUTPUT IN THE BROWSER!
// prompt11();
...
function prompt12() {
  ...
  for(let i=1; i<=10; i++){
    for(let j=11; j<=20; j++){
        console.log(`i: ${i} / j: ${j}`);// this will count from 11-20, 10 times
    }
  }
}
...
function prompt13() {
  // YOUR CODE HERE
  for(let i=0; i< nestedArrays.length; i++){ // traverses first layer
    for(let j=0; j< nestedArrays[i].length; j++){// traverses second layer
