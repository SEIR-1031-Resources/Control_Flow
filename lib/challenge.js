...
function prompt7() {
  ...
  for (let numbers = 0; numbers <= 100; numbers ++) {
    if (numbers % 5 === 0) {
    console.log(`I found a ${numbers}. High five!`)
  }
  }
}
...
let length = order.length; // figuring out the length of the array 
console.log(length);

let half = length / 2; // divide the length of the array be 2 to find the midde of the array 
console.log(half);

let halfFloored = Math.floor(half); //because the middle of the array is 12.5 we need to round to the nearest whole number position
//
let median = order[halfFloored]; // finding the number that is in the middle that is ordered
