...
function prompt11() {
  let order= nums.sort()
  // sorted the array
    let length = order.length // finding the length of the array 
    console.log(length)
    let half = length/2 // dividing the array by 2 to find the middle of the array
   console.log(half);
    let halfFloored= Math.floor(half); // the middle of the array is 12.5 we beed to round to the nearest whole whole number position
  
    let median = order[halfFloored]; // just finding the number that is in the middle 
  
    console.log(order[median]);
