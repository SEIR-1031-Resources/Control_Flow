...
let someNumber = Math.floor(Math.random()*99)

function prompt8(myNum) {
  if (myNum < 30 ){
    console.log(myNum)
    console.log(`That's a small number!`)
  }
  if ((myNum > 30) && (myNum < 60 )){
    console.log(myNum)
...
function prompt11(myArr) {
  let median = myArr[Math.floor(myArr.length/2)];
  console.log(median);
}
// COMMENT IN THE BELOW LINE OF CODE TO TEST YOUR OUTPUT IN THE BROWSER!
// prompt11(nums);
