...
// I saw prompt 5 instructions on accident 
function prompt4() {
  for (i = 10; i >= 0; i--) {
    console.log(i);
  }
...
function prompt8() {
  ...
  } else { // can also use else if (someNumber > 60)
...
function prompt9() {
  for (i = 0; i < starWars.length; i++) {
    console.log(`${starWars.indexOf(starWars[i])}. ${starWars[i]}`);
  }
}
...
function prompt13() {
  for (i = 0; i < nestedArrays.length; i++) {
    for (b = 0; b < nestedArrays[i].length; b++) {
      console.log(nestedArrays[i][b]);
    }
  }
}
