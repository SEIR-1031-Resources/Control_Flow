...
function prompt7() {
  ...
  //* For the numbers 0 - 100 (inclusive), print out 
  for (let i=0; i<=100; i++) {
    //console.log(i) 
   // I found a [ number ]. High five!" if the
   // * number is a multiple of five.
 if (i % 5 === 0) {
...
// IMPORTANT!!!!! code will not run because of error "Uncaught SyntaxError: Identifier 'nums' has already been declared (at challenge.js:1:1)"
// even though theres no identifier there and do not know how to fix it so im commenting it out to avoid issues with other prompts
function prompt11() {
